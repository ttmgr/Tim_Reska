...
def dqs_gauge(dqs: DQSv2Result) -> None:
    ...
    # Gauge
    tier_colors = {"adequate": "#38a169", "caution": "#d69e2e", "insufficient": "#e53e3e"}
    color = tier_colors.get(dqs.tier, "#718096")

    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=dqs.dqs,
        number={"font": {"size": 32, "color": "#1a365d", "family": "Inter, -apple-system, sans-serif"},
                "valueformat": ".2f"},
        gauge={
        ...
        title={"text": f"DQS: {dqs.tier.upper()}",
               "font": {"size": 11, "color": "#718096", "family": "Inter, -apple-system, sans-serif"}},
    ))
    fig.update_layout(
        height=200,
...
def dqs_components_bar(dqs: DQSv2Result) -> None:
    ...
    components = [
        ("Completeness (0.40)", dqs.completeness, "#2b6cb0"),
        ("Consistency (0.35)", dqs.consistency, "#38a169"),
        ("Recency (0.25)", dqs.recency, "#d69e2e"),
        ("Range Score", dqs.range_score, "#2d3748"),
    ]
    ...
    for label, value, color in components:
        ...
        with col1:
            st.markdown(
                f'<div style="font-size:0.7rem; color:#718096; text-transform:uppercase; '
                f'letter-spacing:1px; margin-bottom:2px;">{label}</div>'
                f'<div style="background:#edf2f7; border-radius:3px; height:10px; width:100%; '
        ...
        with col2:
            st.markdown(
                f'<div style="font-size:0.85rem; font-weight:600; color:#1a365d; '
