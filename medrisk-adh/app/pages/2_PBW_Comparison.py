...
if int_dqs.missingness_types:
    patterns = ", ".join(f"{k} ({v})" for k, v in int_dqs.missingness_types.items())
    st.warning(f"**INT missingness pattern:** {patterns} -- these gaps force the model to rely on population priors instead of patient evidence.")
